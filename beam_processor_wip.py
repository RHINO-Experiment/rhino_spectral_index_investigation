...
def return_sky_map(freq_mhz, basemap, betas, base_freq_mhz, t_cmb=2.726, include21cm=False, gaussian_params_21cm=[0,0,0]):
    sky_map = ((basemap - t_cmb)*(freq_mhz / base_freq_mhz) ** betas ) + t_cmb
    if include21cm:
        sky_map += gaussian(freq_mhz, gaussian_params_21cm)
    return sky_map
...
def compute_antenna_temp_mp(args):
    t_obs, freq_mhz, beam_folder, beam_string, ground_temp = args
    ...
    sky_beam_pattern, ground_beam_pattern = read_in_beam_pattern(beam_folder, beam_string, freq_mhz)
    ...
    antenna_temp = np.sum(rotated_map * sky_beam_pattern) + (ground_temp * np.sum(ground_beam_pattern))
...
def gaussian(nu, params):
    mu, a, sigma = params
    return a * np.exp(-((nu - mu)**2) / (2*(sigma**2)))
...
if __name__ == '__main__':
    ...
    parser.add_argument("beam_string", help='.txt String Leader to Parse, e.g beam_string70.0.txt')
    parser.add_argument('beam_folder', help='Path of Beam Folder containing .fits files. e.g beam_folder/beam_string70.0.fits')
