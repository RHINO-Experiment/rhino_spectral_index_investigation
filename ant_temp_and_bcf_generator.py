...
def main(beam_folder, beam_string, save_path, save_name):
    ...
    obs_freqs = [beam_processor.parse_filename(txt, beam_string) for txt in txt_list]
    ...
    bcf_ref_freq_mhz = 70
    bcf_ref_freq_index = np.where(bcf_ref_freq_mhz==obs_freqs_mhz)[0][0]
    ...
    beam_patterns = [beam_processor.read_in_beam_pattern(beam_folder, beam_string, freq) for freq in obs_freqs_mhz]
    print('Beams Loaded...')
    ...
    for a, t in enumerate(time_array):
        rotated_base_map = beam_processor.map_rotator(reference_map, t)
        rotated_beta_map = beam_processor.map_rotator(betas, t)
        bcf_ref_sky_map = beam_processor.return_sky_map(bcf_ref_freq_mhz, rotated_base_map, rotated_beta_map, reference_freq_mhz)
        bcf_ref_sky_temp = np.sum(bcf_ref_sky_map  * beam_patterns[bcf_ref_freq_index])
        bcf_ref_beam_int = np.sum(beam_patterns[bcf_ref_freq_index])
        for b, f_mhz in enumerate(obs_freqs_mhz):
            bcf_int_numerator = np.sum(beam_patterns[b] * bcf_ref_sky_map)
            bcf_int_beam_int = np.sum(beam_patterns[b])
    ...
    for a, t in enumerate(time_array):
        ...
        rotated_beta_map = beam_processor.map_rotator(betas, t)
        for b, f_mhz in enumerate(obs_freqs_mhz):
            sky_map = beam_processor.return_sky_map(f_mhz, rotated_base_map, rotated_beta_map, reference_freq_mhz)
...
if __name__ == '__main__':
    main('/Users/user/Desktop/RHINO/Spectral_Indices_RHINO_Paper/AhmedBeams2025_04_28/SmallHornPatterns/HornDry',
         'HornDry',
         '/Users/user/Desktop/RHINO/Spectral_Indices_RHINO_Paper/SmallHornsTest',
         'HornDry')
